...
ClapTrap::ClapTrap(const std::string& name) : _name(name), _hitPoint(10), _enegyPoint(10), _attackDam(10) {
	std::cout << "ClapTrap Argument constructor called: " << this->_name << std::endl;
}
...
ClapTrap& ClapTrap::operator=(const ClapTrap& rhs) {
	...
	std::cout << "ClapTrap Operator assigned called" << std::endl;
	return *this;
}
ClapTrap::~ClapTrap() {
...
void ClapTrap::attack(const std::string& target) {
	...
	{
		std::cout << "ClapTrap " << this->_name << " can't attack: Insufficient energy points" << std::endl;
		return;
	}
	this->_enegyPoint -= 1;
	std::cout << "ClapTrap " << this->_name 
